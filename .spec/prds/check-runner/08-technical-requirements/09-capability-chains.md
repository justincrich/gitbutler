...
head)` that **calls no runner entry point** (`but check run` / `runner.rs`)
    ...
    actually supports. **Caveat:** literally _observing_ "no child process was
...
| 2. Evaluate before-state | gate evaluates the required set **in force before** the weakening commit | Ref-pin is necessary but not sufficient — evaluate the pre-change set (R-BOOTSTRAP) | Evaluating the _proposed_ (weakened) set would let it self-escalate | Weakening commit blocked until currently-required checks pass |
...
- **Limitation (state it alongside the property — do not over-claim):** the
  ...
  removes**, but does **NOT** force human review of the weakening _itself_. An
