...
Baseline SHA: a80ce888a894ae3568ee8127866e987e89d6c092 (AUTHZ-004: harden merge gate fail closed). AC-2/TC-3 use `./tools/governance-checks/check_merge_gate_production_unchanged.sh`, which enforces a zero-diff baseline-range check for `crates/but-api/src/legacy/merge_gate.rs` and also fails on staged or unstaged changes to that file.
...
- [ ] AC-2: GATES-008 adds NO competing merge-path fail-closed check — merge_gate.rs production classification (config.invalid/perm.denied/undefined-group) is UNCHANGED; AUTHZ-004 remains the owner (build-gate, zero-diff baseline script)
...
AC-2: GATES-008 adds NO competing merge-path fail-closed check — AUTHZ-004 remains the owner [build-gate]
  ...
  WHEN:  the diff for this task is structurally inspected with the zero-diff baseline script
  ...
  TEST_TIER: unit (build-gate)   VERIFICATION_SERVICE: git zero-diff baseline + compile   UNIT_TEST_JUSTIFIED: the ownership/no-duplication invariant (production classification unchanged) is a zero-diff baseline/compile build-gate with zero runtime I/O; the behavioral target-ref-only property is proven by AC-1 integration cases
...
- TC-2 (-> AC-1, happy_path): after a distinct approval @head per the TARGET-REF requirement, the merge is permitted (the target-ref requirement, once satisfied, proceeds — NOT the feature-head min_approvals=0 trivially passing)
    VERIFY: cargo test -p but-api merge_gate_targetref_only_feature_head_drop_ignored
- TC-3 (-> AC-2, structural): GATES-008 modifies NO production merge-path classification logic (merge_gate.rs production unchanged); AUTHZ-004 remains the owner (no duplicate undefined-group/malformed check) — zero-diff baseline script
    VERIFY: ./tools/governance-checks/check_merge_gate_production_unchanged.sh
...
- NO competing merge-path classification change (zero-diff baseline script, S1): `./tools/governance-checks/check_merge_gate_production_unchanged.sh` -> Exit 0 (no production merge-gate diff from AUTHZ-004 baseline; AUTHZ-004 remains the owner)
...
<!--
{
  ...
  "notes": [
    ...
    "S1 (CRITICAL, fixed): the AC-3 verify + AC-3 scenario must_observe previously used a logically-INVERTED command (git diff --name-only | grep -qvE '^...merge_gate.rs$') that passes trivially whenever ANY test file changes. BOTH are now enforced by `./tools/governance-checks/check_merge_gate_production_unchanged.sh`, which checks `crates/but-api/src/legacy/merge_gate.rs` against Baseline SHA a80ce888a894ae3568ee8127866e987e89d6c092 and fails on committed, staged, or unstaged changes. The (now AC-2) scenario must_observe/must_not_observe describe THAT check.",
  ...
  "requirements": [
    ...
    },
    {
      ...
      "type": "acceptance_criterion",
      "primary": false,
      "description": "GIVEN AUTHZ-004 owns the merge-path fail-closed classification (config.invalid/perm.denied/gate.review_required ordering + the undefined-required-group hard-deny) in merge_gate.rs production code WHEN this task's diff is structurally inspected with the zero-diff baseline script THEN this task modifies NO production classification logic in crates/but-api/src/legacy/merge_gate.rs (the change set is test files + the GateConfig fixture helper only) — AUTHZ-004 remains the owner, GATES-008 is a standalone proof, not a competing owner",
      "verify": "./tools/governance-checks/check_merge_gate_production_unchanged.sh",
      "maps_to_ac": null,
      "scenario": {
        "tier": "holdout",
        "test_tier": "unit",
        "unit_test_justified": "the ownership/no-duplication invariant (production classification unchanged, AUTHZ-004 remains owner) is a zero-diff baseline/compile build-gate with zero runtime I/O; the behavioral target-ref-only property is proven by the AC-1 integration cases",
        "verification_service": "git zero-diff baseline + compile (build-gate, no runtime I/O)",
        "negative_control": {
          "would_fail_if": [
            "this task adds a competing undefined-group / malformed-config classification in merge_gate.rs production code (duplicating AUTHZ-004) — the zero-diff baseline script finds committed, staged, or unstaged changes",
        ...
        "cases": [
          {
            ...
            "action": {
              "actor": "ci",
              "steps": [
                "Run `./tools/governance-checks/check_merge_gate_production_unchanged.sh` to confirm GATES-008 adds NO competing classification logic relative to Baseline SHA a80ce888a894ae3568ee8127866e987e89d6c092 (cite AUTHZ-004 OUT-OF-SCOPE: GATES-008 is the deepening, not a competing owner)"
              ]
            },
            "end_state": {
              "must_observe": [
                "`./tools/governance-checks/check_merge_gate_production_unchanged.sh` exits 0 (no committed, staged, or unstaged production classification change relative to Baseline SHA a80ce888a894ae3568ee8127866e987e89d6c092)",
    ...
    { "id": "TC-3", "type": "test_criterion", "description": "GATES-008 modifies NO production merge-path classification logic (merge_gate.rs production unchanged via zero-diff baseline script); AUTHZ-004 remains the owner (no duplicate undefined-group/malformed check)", "verify": "./tools/governance-checks/check_merge_gate_production_unchanged.sh", "maps_to_ac": "AC-2" },
