...
- `crates/but-api/tests/group_governance.rs` (NEW) — the PRIMARY proofs: admin membership write lands inert in the working tree (ref*id unchanged); non-admin group op denied; create/grant land in the working tree preserving unrelated entries; group_grant fail-closed (undefined group / bad token / unset handle); list under admin:read (composes `but-api` `group*\*`against real`but-authz`+ real gix via`but_testsupport::writable_scenario`).
