...
- crates/gitbutler-tauri/tests/mgmt*ipc_003_governance_commands.rs (MODIFY — add `principal_kind_read`/`principal_kind_update` to the `GOVERNANCE_COMMANDS` const + an `InvocationCase` for each, so the real Tauri mock-runtime bus proves both register and invoke; the existing forbidden-`allow-governance*\*`-file assertion already covers the capability surface)
