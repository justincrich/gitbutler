...
Generated by /kb-sprint-tasks-plan on 2026-06-22T18:00:00Z from `.spec/reviews/red-hat-20260622-173510.md`.
The original LPR slice shipped with 4 CRITICAL + 3 HIGH gaps; these tasks close them. Each task references
the red-hat finding it remediates and carries its own stable `AC-N`/`TC-N` Requirement Contract.

| ID          | Title                                                                                                                                   | Agent                                    | Est     | Remedies      |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- | ------- | ------------- |
| REM-LPR-004 | Re-point `but review comment` → `post_comment`; add Comments/Resolve CLI verbs + `--file/--line/--thread`; delete dead `comment_review` | rust-implementer / rust-reviewer         | 60 min  | C1 (CRITICAL) |
| REM-LPR-009 | Complete the safe-seam proof — move grep into `invariant_build_gates.rs` + bidirectional forged-vs-empty + 3-step capstone              | rust-reviewer                            | 90 min  | C2 (CRITICAL) |
| REM-LPR-010 | Add `lpr_review_cli_happy_path` snapbox + `lpr_full_local_loop_request_to_merge_no_forge` e2e capstone + write `NAPI-AUDIT.md`          | rust-implementer / rust-reviewer         | 120 min | M1+L1         |
| REM-LPR-014 | Surface `kind` in `governance_principals_list` (extend DTO + SDK regen)                                                                 | rust-implementer + sveltekit-implementer | 45 min  | M2 (HIGH)     |
| REM-LPR-016 | Wire `<LocalReviewView>` into `GovernanceSettings.svelte` (5th tab) + extend `ReviewStatus` with PR-header fields                       | sveltekit-implementer + rust-implementer | 90 min  | C3+M3         |

### Remediation task files

- REM-LPR-004-cli-comment-comments-resolve-wiring.md
- REM-LPR-009-safe-seam-proof-completion.md
- REM-LPR-010-cli-happy-path-and-e2e-capstone.md
- REM-LPR-014-surface-kind-in-principals-list.md
- REM-LPR-016-mount-local-review-view-tab.md

### Remediation dependency chain

```
REM-LPR-004 (CLI wiring) ──────────────────────→ REM-LPR-010 (snapbox suite + e2e capstone)
REM-LPR-009 (safe-seam proof) ─────────────────→ (blocks sprint closeout — load-bearing)
REM-LPR-014 (DTO extension + SDK regen) ────────→ (independent; can land in parallel)
REM-LPR-016 (UI mount + backend DTO) ──────────→ (independent; can land in parallel)
```

> **Closeout gate**: the slice is "done" when REM-LPR-009 is green (load-bearing per SPRINT.md "If LPR-009
> is not green, the slice is NOT done") AND REM-LPR-004 + REM-LPR-010 land (closing the CLI theatre +
> landing the central thesis proof). REM-LPR-014 + REM-LPR-016 may follow in a second wave if needed.
