...
> **Principals have permission sets, not roles.** "Implementer" and "reviewer" anywhere in this PRD are *illustrative labels* for common permission bundles — a user may grant any combination and name a principal anything (`coder`, `code-checker`, `security-bot`). A principal *is* the `AuthoritySet` it holds; nothing in enforcement keys off a name (UC-AUTHZ-02 AC, grep-asserted). The built-in `read/triage/write/maintain/admin` presets are GitHub-mirrored sugar that desugar to functional sets. **Out of scope this slice:** *user-defined* named permission templates.
