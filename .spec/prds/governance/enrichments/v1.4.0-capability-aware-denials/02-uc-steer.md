...
The **STEER** group turns each governance denial into a flow-diverter. Where AUTHZ/GRPS/GATES decide *whether* an action proceeds, STEER governs *what the denial communicates back* so the caller re-routes itself down the governed path. It is additive: every STEER behavior layers onto the existing denial carriers — `Denial { code, message, remediation_hint }` and `MergeGateError` (which additionally carries `unmet`) — without changing a single gate decision.
