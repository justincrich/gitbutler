...
> because `but` shows an *open assignment*; it triggers remediation because `but` shows an *unresolved
> comment*; it merges because `but` shows an *approved verdict at head*. Every decision is a projection of
...
```jsonc
{"local_pr":{
  "target":"feature/login", "source_branch":"feature/login", "sha":"a1b2c3d",   // ForgeReview-shape fields
  "author":"rust-implementer", "title":"Add login flow", "draft":false,
  ...
  "assignments":[                                                                // local_review_assignments — drive-only, NEVER gates; reviewer must be distinct from author
    {"reviewer":"rust-reviewer","state":"pending","assigned_at":"2026-06-21T12:00:00Z"}
  ],
  "comment_threads":[                                                            // local_review_comments — drive-only, NEVER gates; resolve constrained to author/reviewer/reviews:write
    {"thread_id":"t1","file":"src/login.rs","line":42,"resolved":false,
