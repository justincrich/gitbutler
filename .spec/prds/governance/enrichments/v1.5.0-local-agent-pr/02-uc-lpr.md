...
- ☐ A human maintainer can set a per-project "keep PRs local" flag as **a per-project operator preference, default-local, under the R12 trusted-desktop model — NOT functional-permission-gated** (it is a `gitbutler_project::Project` setting persisted in the project store, the same class as `forge_override`/`preferred_forge_user`, set via the desktop project-settings surface — _not_ a ref-pinned `.gitbutler/{permissions,gates}.toml` governed-config change, and _not_ gated by `administration:write`). The desktop human is the trusted fleet owner (R12), so the preference is owned by implicit desktop ownership rather than a checked grant; an untrusted write to the project store that flips it (→ agent PRs mirror to a public forge) is the named accepted residual **R21 (§G in [03](./03-technical-requirements-delta.md))** — the same accepted-leak class as R12.
