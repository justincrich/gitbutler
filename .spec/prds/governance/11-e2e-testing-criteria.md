...
| T-LOOP-004 | maintainer merges only after requirement met                                                | AC-4                  | integration-test | merge holder                     | gate permits + execution reaches the `merge_review` boundary post-approval (forge-network completion structural — forge-locality accepted limitation)                                                                                                                                                                                                                                                                                   |
...
| T-LOOP-006 | full-loop integration test (the reference flow)                                             | AC: full-loop         | integration-test | real surface+git, 3 principals   | all five loop assertions hold (reviews via governed `but review`); the maintainer-merge assertion proves the gate permits + execution reaches the `merge_review` boundary (forge-network completion structural — forge-locality)                                                                                   |
...
| T-LOOP-010 | both approvals @head → merge proceeds                   | AC-4   | integration-test | both groups approve          | gate permits + reaches the `merge_review` boundary (forge completion structural) |
...
| T-LOOP-012 | two-tier integration test (only-one blocked, both pass) | AC-6   | integration-test | real surface+git             | AI-only blocked; human-only blocked; both → gate permits + reaches the `merge_review` boundary |
