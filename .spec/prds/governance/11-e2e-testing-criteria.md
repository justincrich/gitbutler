...
- Criteria are tallied **by test type, not by risk severity** — R6's Medium→High upgrade and R14 add no ripple to the type counts (the type deltas vs v1.2.0 are the +2 build-gates for the N-API audit and the desktop CT config, +3 integration for the new make-explicit/lifecycle/bootstrap behaviors, and +4 component for the new MGMT UI behaviors; type *re-labels* of a few existing rows reflect their true assertion class — contract-shape → api-contract, structural/grep → build-gate — and net to the canonical 67/38/7/15/2 split).
