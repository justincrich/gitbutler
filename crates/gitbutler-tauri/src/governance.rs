...
pub fn branch_gates_read_for_desktop_session(
    ...
    let (ctx, owner, _cap) = fleet_owner_context(session, project_id, &target_ref)?;
    let repo = ctx.repo.get().map_err(json::Error::from)?;
...
pub fn branch_gates_update_for_desktop_session(
    ...
    let (ctx, owner, cap) = fleet_owner_context(session, project_id, &target_ref)?;
    let repo = ctx.repo.get().map_err(json::Error::from)?;
...
///
/// Re-exported from `but-api` so the desktop Tauri command and the SDK share
/// one canonical DTO (the spec's "strictly site at the but-api boundary"
/// invariant). The read DTO (`PrincipalKindList`) and the write outcome both
/// live in `but_api::legacy::governance`.
pub use but_api::legacy::governance::PrincipalKindOutcome as PrincipalKindWriteOutcome;

/// Invoke `principal_kind_update` as the signed-in desktop fleet-owner.
///
/// The but-api fn (`principal_kind_update_with_repo`) composes the AUTHZ-006
/// `enforce_administration_write_gate` for the env-principal path. The desktop
/// wrapper instead resolves the human fleet-owner via `DesktopSessionState`
/// (`BUT_AGENT_HANDLE` is unset on desktop) and calls the `_as_fleet_owner`
/// variant, EXACTLY like `branch_gates_update_for_desktop_session`. The write
/// lands in the WORKING-TREE `permissions.toml` and takes effect once
/// `governance_commit` commits it to the target ref.
pub fn principal_kind_update_for_desktop_session(
    session: &dyn DesktopSession,
    project_id: ProjectHandleOrLegacyProjectId,
    target_ref: String,
    principal: String,
    ...
    let (ctx, _owner, _cap) = fleet_owner_context(session, project_id, &target_ref)?;
    let repo = ctx.repo.get().map_err(json::Error::from)?;
    but_api::legacy::governance::principal_kind_update_with_repo_as_fleet_owner(
        &repo,
        &target_ref,
        &principal,
        &kind,
...
/// Tauri command wrapper for desktop fleet-owner `principal_kind_update`.
///
/// The read command (`principal_kind_read`) rides the but-api
/// `#[but_api(napi)]`-generated `tauri_principal_kind_read::principal_kind_read`
/// module on `core:default` like `perm_list`; only the write needs the
/// fleet-owner shim, so only the write carries a hand-written wrapper here.
pub mod tauri_principal_kind_update {
    use super::{
    ...
    pub fn principal_kind_update(
        ...
        super::principal_kind_update_for_desktop_session(
            desktop_session.session(),
            project_id,
            target_ref,
            principal,
