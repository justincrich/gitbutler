...
fn agent_primer_is_non_empty() {
    assert!(!AGENT_PRIMER.trim().is_empty(), "AGENT_PRIMER must contain reference text");
}
...
fn agent_primer_covers_contract_tokens() {
    ...
    let missing: Vec<&str> = ["actor_correctable", "operator_required", "authorized_actions", "do_not"]
        .into_iter()
        .filter(|token| !lowercase.contains(token))
        .collect();
    assert!(missing.is_empty(), "AGENT_PRIMER must mention all contract tokens, missing: {missing:?}");
}
